...
function clientLabel(n) {
	return n === 1 ? "1 client" : `${formatCount(n)} clients`;
}
var buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium outline-none select-none disabled:pointer-events-none disabled:opacity-40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 focus-visible:ring-2 focus-visible:ring-ring/70 focus-visible:ring-offset-2 focus-visible:ring-offset-bg active:not-disabled:scale-[0.96] transition-[scale,background-color,color,opacity] duration-150 ease-out", {
	variants: {
		variant: {
...
function displayName(id, rawName) {
...
function countriesWithClients(counts) {
	let n = 0;
...
var worldCountries = feature_default(topo, topo.objects.countries);
function countryId(feature) {
	if (feature.id !== void 0 && feature.id !== null && String(feature.id).length > 0) return String(feature.id).padStart(3, "0");
...
function CountryRail({ className }) {
	...
	const rows = (0, import_react.useMemo)(() => {
		...
		return Object.keys(COUNTRY_NAMES).map((id) => ({
			id,
			name: countryName(id),
			n: counts[id] ?? 0
		})).filter((row) => q ? row.name.toLowerCase().includes(q) : row.n > 0).sort((a, b) => b.n - a.n || a.name.localeCompare(b.name));
	}, [counts, query]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
		className: cn("flex h-full min-h-0 flex-col bg-surface shadow-[var(--shadow-border)]", className),
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("ul", {
				...
				children: [rows.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", {
					...
					return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						...
						className: cn("flex w-full flex-col gap-1.5 rounded-md px-3 py-2.5 text-left", "transition-[background-color] duration-150 ease-out", on ? "bg-surface-2" : "hover:bg-surface-2/60"),
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
							className: "flex items-baseline justify-between gap-3",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: cn("text-sm", on ? "text-fg" : "text-fg/90"),
								children: row.name
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "font-mono text-xs tabular-nums text-muted",
								children: formatCount(row.n)
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "h-0.5 overflow-hidden rounded-full bg-fg/8",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
...
function Stat({ label, value }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "rounded-md bg-surface-2 px-3 py-2.5 shadow-[var(--shadow-border)]",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
			className: "text-xs text-subtle",
			children: label
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
			className: "mt-0.5 font-mono text-lg tabular-nums text-fg",
			children: value
		})]
	});
}
var SKIP = /* @__PURE__ */ new Set(["010", "260"]);
function buildWorldMap(collection, width, height) {
	const w = Math.max(1, Math.floor(width));
	const h = Math.max(1, Math.floor(height));
	if (w < 48 || h < 48) return null;
	const features = collection.features.filter((f) => !SKIP.has(countryId(f)));
	const padX = Math.max(12, w * .03);
	const padY = Math.max(10, h * .05);
	const projection = naturalEarth1_default().fitExtent([[padX, padY], [w - padX, h - padY]], {
	...
	const dy = spacing * .86;
	const dots = [];
	for (let row = 0, y = spacing; y < h - spacing * .5; row += 1, y += dy) {
		const x0 = spacing * .6 + (row % 2 === 1 ? spacing * .5 : 0);
		for (let x = x0; x < w - spacing * .5; x += spacing) {
...
function hitCountry(model, x, y) {
	...
	if (!n) {
		const r = 5;
		outer: for (let dy = -5; dy <= r; dy++) for (let dx = -5; dx <= r; dx++) {
...
function drawMap(ctx, model, counts, hovered, selected, cssW, cssH, dpr) {
	...
	for (const d of dots) {
		...
		const t = n <= 0 ? 0 : .38 + .62 * Math.sqrt(n / max);
		const col = n <= 0 ? LAND : mix(LAND, ACCENT, Math.min(1, t));
		let alpha = n <= 0 ? .13 : .28 + .72 * t;
		if (isDim) alpha *= .22;
		if (isFocus) alpha = n > 0 ? 1 : .55;
		const r = isFocus ? n > 0 ? 2.15 : 1.7 : n > 0 ? 1.35 + .7 * t : 1.2;
		ctx.beginPath();
		ctx.fillStyle = isFocus ? `rgba(${HOVER.r},${HOVER.g},${HOVER.b},${alpha})` : `rgba(${col.r},${col.g},${col.b},${alpha})`;
...
function DottedMap() {
	...
	useClientsStore((s) => s.setSelected);
	const setHovered = useClientsStore((s) => s.setHovered);
	const countsRef = (0, import_react.useRef)(counts);
	const selectedRef = (0, import_react.useRef)(selectedId);
	const hoveredRef = (0, import_react.useRef)(hoveredId);
	...
	const paint = (0, import_react.useCallback)(() => {
		...
		drawMap(ctx, model, countsRef.current, hoveredRef.current, selectedRef.current, rect.width, rect.height, dpr);
	}, []);
	...
	const updateHover = (clientX, clientY, pin) => {
		const wrap = wrapRef.current;
		const model = modelRef.current;
		...
		if (pin) useClientsStore.setState({
			selectedId: id,
			hoveredId: id
		});
		paint();
	...
	const share = tooltip && total > 0 && n > 0 ? Math.round(n / total * 1e3) / 10 : 0;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		ref: wrapRef,
		...
		children: [
			...
			hint && !tooltip && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "pointer-events-none absolute bottom-4 left-1/2 z-10 -translate-x-1/2 px-4 text-center text-sm text-muted atlas-in",
				children: "Move across a country to read its client count"
			}),
			tooltip && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				style: {
					left: tooltip.x,
					top: tooltip.y,
					transform: tooltip.y < 96 ? "translate(-50%, 14px)" : "translate(-50%, calc(-100% - 14px))"
				},
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "font-display text-lg leading-snug text-fg",
						children: countryName(tooltip.id)
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-1 font-mono text-sm tabular-nums text-accent",
						children: clientLabel(n)
					}),
					n > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "mt-0.5 text-xs text-subtle",
						children: [share, "% of all clients"]
					})
				]
			})
		]
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center gap-3 atlas-in",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "grid size-9 place-items-center rounded-md bg-surface shadow-[var(--shadow-border)]",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Earth, { className: "size-4 text-accent" })
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
						className: "font-display text-xl leading-none tracking-tight sm:text-2xl",
						children: "Client Atlas"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-1 hidden text-xs text-muted sm:block",
						children: "Hover any dot to read the count"
					})] })]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DottedMap, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Legend, {})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CountryRail, { className: "hidden w-[340px] shrink-0 rounded-tl-xl lg:flex" })]
			}),
			open && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					...
					className: "absolute inset-y-0 right-0 flex w-[min(100%,340px)] flex-col bg-surface",
					...
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CountryRail, { className: "min-h-0 flex-1" })]
				})]
			})
		]
	});
...
function Legend() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "pointer-events-none absolute bottom-3 left-3 hidden items-center gap-4 rounded-md bg-surface/90 px-3 py-2 text-xs text-muted shadow-[var(--shadow-border)] sm:flex atlas-in atlas-in-delay-2",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
				className: "flex items-center gap-1.5",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "size-1.5 rounded-full bg-fg/25" }), "Land"]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
				className: "flex items-center gap-1.5",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "size-1.5 rounded-full bg-accent/70" }), "Clients"]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
				className: "flex items-center gap-1.5",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "size-2 rounded-full bg-accent" }), "Concentrated"]
			})
		]
	});
}
